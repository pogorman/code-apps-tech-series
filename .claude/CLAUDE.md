...
A floating chat panel (`src/components/copilot-chat.tsx`) embeds a Copilot Studio agent via iframe. The agent URL is `https://copilotstudio.microsoft.com/environments/0582014c-9a6d-e35b-8705-5168c385f413/bots/auto_agent_s82bp/webchat?__version__=2`. SSO is handled natively by the Power Platform host — no Direct Line secrets, MSAL, or `botframework-webchat` dependency. The component renders alongside `CommandPalette` in `App.tsx` (outside `AppLayout`, inside `HashRouter`). Blue gradient floating button (bottom-right), 400x600 panel, refresh/close in header. Dark mode aware via `useTheme()`. This is the same agent used in the `dv-front-end` repo, but embedded via iframe instead of Direct Line because the Code App's `@microsoft/power-apps` SDK uses custom `paauth`/`dynamicauth` tokens incompatible with Direct Line's SSO token exchange.
