# seed-data.ps1 - Bulk-create demo data in og-dv commercial environment
# Mirrors the gcc seed-data script so commercial test data matches gcc.
# Uses Azure CLI access token + Dataverse Web API.
# Prereq: az login to the M365x06150305 tenant (admin@M365x06150305.onmicrosoft.com)
# Run: powershell -ExecutionPolicy Bypass -File scripts/seed-data.ps1

$ErrorActionPreference = "Continue"
$env_url = "https://og-dv.crm.dynamics.com"
...
if (-not $token -or $token -like "ERROR*") {
    Write-Error "Failed to get auth token. Run: az login --scope https://og-dv.crm.dynamics.com/.default"
    exit 1
}
