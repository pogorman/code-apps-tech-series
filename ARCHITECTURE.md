...
- **`dashboard/`** — Two views: (1) Analytics dashboard (`dashboard.tsx`) with framer-motion `motion.div` entrance animations (shared `riseInitial → riseAnimate` presets, staggered per index), glassmorphism KPI cards and `ChartCard` panels (shared `GLASS_CARD` class: `bg-card/60 backdrop-blur-xl` with layered shadow), soft radial accent glows behind each chart card's glass, accent-bordered KPI cards (left colored border, radial accent glow, icon in tinted badge), and `ChartCard` panels (top gradient accent line, vertical bar section indicator, frosted-glass tooltips). SVG donut (144px, 18px stroke, background track), an animated **Recharts** `BarChart` (horizontal/`layout="vertical"`) for the Priority Distribution card with per-priority `<Cell>` colors, 850ms ease-out animation, frosted-glass custom tooltip, and bar click-through to drilldown, segmented task-type overview bar with per-type icons. Hover tooltips preview underlying data; click any tile or sub-element to open a drilldown dialog (`drilldown-dialog.tsx`) showing a filtered table of action items. (2) Kanban board (`board-dashboard.tsx`) with framer-motion entrance animations on columns and every card (outer dnd-kit node keeps its transform; motion wraps the *inner* card content so entrance and drag never fight), glassmorphism columns (`bg-card/55 dark:bg-card/35 backdrop-blur-xl`) and semi-translucent glass cards (`bg-card/75 backdrop-blur-md`), and four drag-and-drop columns: parking lot (pinned items from any entity, green accent + Car icon), work (wider 2fr column with dynamic accent/icon/title based on active task-type filter), projects (`tdvsp_project` records, purple accent), and ideas (amber accent). Grid layout: `grid-cols-[1fr_2fr_1fr_1fr]`. Uses a single `@dnd-kit` `DndContext` with custom `CollisionDetection` (closestCenter for within-column reorder, pointerWithin for cross-column drops); `useDroppable` on columns; cross-column drag pins/unpins items via `tdvsp_pinned` field. Active drop-target columns highlight with an accent-colored ring, border, and glow. All cards are clickable — clicking opens the entity's edit form dialog. Floating `CardToolbar` on hover (compact: backdrop-blur, smaller padding) provides GripVertical (drag), color dots (priority), Pencil (edit), and Car icon (park/unpark) controls. Cards have hover lift (`-translate-y-0.5`), graduated shadows, entity-type icons inline with titles, 1-line description snippets, and priority-tinted gradient backgrounds via `tileGradient()` (dark-mode aware). Drag state adds `scale-[1.03]`, `rotate-[1.5deg]`, `ring-2 ring-primary/40` with elevated `z-index: 9999`. Glass-morphism sticky column headers (`backdrop-blur-xl`, `bg-background/60`) with overlapping count badges. Outline-style priority/status/category pills (`priorityPillClass()`, `statusPillClass()`, `categoryPillClass()`) replace Badge components. Sort order persists in localStorage. `SortableCard` takes an `index` prop so the per-card entrance animation can stagger (`min(index, 10) * 40ms`) — cap prevents sluggish long columns
...
- **`copilot-chat.tsx`** — Floating Copilot Studio agent #1 button. Opens the Copilot Studio hosted webchat in a popup window (`window.open`) so the agent handles its own auth natively. No iframe, no Direct Line, no MSAL, no `botframework-webchat`. Blue gradient floating button (bottom-right, `right-6`, `MessageCircle` icon). Clicking opens a popup; clicking again focuses the existing popup or opens a new one if closed
- **`copilot-chat-panel.tsx`** — Floating Copilot Studio agent #2 button + in-app chat panel. Calls `MicrosoftCopilotStudioService.ExecuteCopilotAsyncV2()` from the generated connector service with body `{ notificationUrl, message }` and optional `x_ms_conversation_id` for multi-turn. Auth is inherited from the Power Apps host's Entra ID session — zero token handling. Purple/pink gradient button (`right-[5.5rem]`, `Sparkles` icon) sits immediately to the left of `copilot-chat.tsx`. Click toggles a 384×560 panel anchored `bottom-24 right-6` with message bubbles, a typing indicator (`Loader2` spinner), a Reset button to start a new conversation, and Enter-to-send. Agent schema name hardcoded at top of file (`AGENT_SCHEMA_NAME = "cr6bd_agentDVkdZi"`). Response shape cast from `IOperationResult<void>` — reads `lastResponse` → `responses[]` → `message` from `result.data`, and persists `conversationId` in a ref for the next turn
...
- `services/MicrosoftCopilotStudioService.ts` — Typed connector operations for the `shared_microsoftcopilotstudio` connector. Provides `ExecuteCopilotAsyncV2(Copilot, body, x_ms_conversation_id?, environmentId?)` for agent invocation plus several other operation variants. Backed by `.power/schemas/microsoftcopilotstudio/microsoftcopilotstudio.Schema.json`
...
- `models/MicrosoftCopilotStudioModel.ts` — Copilot Studio connector response types. Includes `ExecuteAgentJsonResponse { message? }`. Note: the runtime response actually has the shape `{ lastResponse, responses[], conversationId }` from the connector schema's `x-ms-notification-content` block, not the typed `ExecuteAgentJsonResponse` — the panel component handles both
...
| Copilot Studio via popup window | Agent #1 opens in a popup window (`window.open`) instead of an iframe or Direct Line embed. The Code App's `@microsoft/power-apps` SDK uses custom `paauth`/`dynamicauth` tokens incompatible with Direct Line's SSO token exchange, and iframe embedding had CSP/auth friction. A popup window lets Copilot Studio handle its own auth natively with zero dependencies |
| Second agent via Copilot Studio connector | Agent #2 uses the `shared_microsoftcopilotstudio` Power Platform connector as a data source (added via `pac code add-data-source -a "shared_microsoftcopilotstudio" -c <connectionId>`), which auto-generates `MicrosoftCopilotStudioService`. Calling `ExecuteCopilotAsyncV2` invokes the agent through the Power Apps host, which inherits the user's Entra ID session — the `dataverse-backed/authenticated/bots/...` endpoint path in the connector schema is the tell. Zero token management in the component. Requires the agent to be set to Microsoft (Entra ID) auth in Copilot Studio Settings → Security. This pattern replaces the need for Direct Line entirely and lets us build a native chat UI inside the app |
| Recharts for the Priority Distribution chart | Recharts was added specifically so that one chart card (Priority Distribution) could use real animated bars with interactive tooltips and click-through to drilldown. The rest of the dashboard (donut, bars, account rows) stays on hand-rolled SVG + Tailwind to keep the bundle lean — Recharts is only imported where its value (animation + native tooltip) outweighs its weight. The custom tooltip component reuses the same frosted-glass styling as the app's `Tip` component for visual consistency |
| Framer Motion for entrance animations | Replaced the CSS `@keyframes dashRise` blocks on both dashboard files with `motion.div` wrappers using shared `riseInitial/riseAnimate` presets. This lets us compose entrance animations with hover states cleanly and makes stagger delays data-driven (per index). On the kanban board, the motion wrapper goes *inside* `SortableCard` (wrapping the child content) instead of outside, so `@dnd-kit`'s transform on the outer node is never competing with a framer transform — entrance only runs on mount, dnd-kit owns the transform during drag |
