...
- **`hvas/`** — High Value Activity list (table), detail dialog, form dialog, delete confirmation; customer (account) lookup
...
- **`dashboard/`** — Action item analytics with KPI cards and Chart.js charts (status, priority, type, by-account)
- **`layout/`** — App shell with "Account Management" title bar and fixed-width (100px) horizontal nav tiles (Dashboard, Accounts, Contacts, Action Items, HVAs, Meetings, Ideas)
...
- `use-hvas.ts` — TanStack Query hooks wrapping `Tdvsp_hvasService` (CRUD + cache invalidation)
...
- `services/AccountsService.ts`, `services/ContactsService.ts`, `services/Tdvsp_actionitemsService.ts`, `services/Tdvsp_hvasService.ts`, `services/Tdvsp_meetingsummariesService.ts`, `services/Tdvsp_ideasService.ts` — Typed CRUD operations
- `models/AccountsModel.ts`, `models/ContactsModel.ts`, `models/Tdvsp_actionitemsModel.ts`, `models/Tdvsp_hvasModel.ts`, `models/Tdvsp_meetingsummariesModel.ts`, `models/Tdvsp_ideasModel.ts` — Dataverse entity types
...
HashRouter (`/#/accounts`, `/#/contacts`, `/#/action-items`, `/#/hvas`, `/#/meeting-summaries`, `/#/ideas`) — required by Power Platform host iframe which controls the outer URL.
...
| Customer lookup via OData bind | Action items and HVAs use `tdvsp_Customer@odata.bind` with `/accounts(guid)` format for writes, `_tdvsp_customer_value` for reads — same polymorphic pattern as contacts |
