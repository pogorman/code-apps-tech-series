...
| Board (Kanban) view | A cross-entity Kanban board showing 3 drag-and-drop sortable columns: work (Work-type action items), projects (`tdvsp_project` records), ideas. Uses `@dnd-kit` for drag-and-drop sorting within columns; sort order persists in localStorage |
