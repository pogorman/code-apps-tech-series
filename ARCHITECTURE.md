...
- `services/AccountsService.ts`, `services/ContactsService.ts`, `services/Tdvsp_actionitemsService.ts` — Typed CRUD operations
- `models/AccountsModel.ts`, `models/ContactsModel.ts`, `models/Tdvsp_actionitemsModel.ts` — Dataverse entity types
...
HashRouter (`/#/accounts`, `/#/contacts`, `/#/action-items`) — required by Power Platform host iframe which controls the outer URL.
