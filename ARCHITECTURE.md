...
- **`copilot-chat.tsx`** — Floating Copilot Studio agent panel. Embeds the agent via iframe from the same Power Platform environment (`0582014c-9a6d-e35b-8705-5168c385f413`). SSO is handled natively by the platform host — no Direct Line secrets, MSAL, or token exchange needed. Floating blue gradient button (bottom-right) opens a 400x600 chat panel with header (refresh + close) and the agent iframe. Dark mode aware via `useTheme()`. Conversation reset via `iframeKey` state (remounts iframe)
...
| Copilot Studio via iframe | The agent is in the same Power Platform environment, so iframe embed gives native SSO without Direct Line/MSAL. The Code App's `@microsoft/power-apps` SDK uses custom `paauth`/`dynamicauth` tokens that aren't compatible with Direct Line's SSO token exchange — iframe sidesteps this entirely |
